				...
				elems: opt.item.map(function (x, ix) {
					...
					return ns.div("accordion-item", [
						...
						ns.div({
							...
							elems: ns.div("accordion-body", x.elems),
						}),
					]);
				}),
			});
...
(function (ns) {
	ns.btngroup = function (opt) {
		if (Array.isArray(opt) || typeof opt === "string") {
			...
			return ns.div({
				class: [opt.vertical ? "btn-group-vertical" : "btn-group", opt.weight ? `btn-group-${opt.weight}` : null, opt.class ? opt.class : null],
	...
	ns.card = {
		...
		imgoverlay: function (opt) {
			if (Array.isArray(opt) || typeof opt === "string") {
				...
				return ns.div(["card-img-overlay", opt.textcolor ? `text-${opt.textcolor}` : null, opt.class], opt.elems);
			}
		},
		...
		build: function (opt) {
			...
			if (typeof opt.button === "string") {
				...
				if (Array.isArray(opt.button)) {
					...
					resFooter.push(
						ns.div("container-fluid g-0 p-0", ns.div("row align-items-center", [ns.div("col", resControl), ns.div("col-auto", resButton)]))
					);
...
(function (ns) {
	ns.dropdown = function (opt) {
		...
		var menu = ns.div({
			class: ["dropdown-menu", opt.dark ? "dropdown-menu-dark" : null, ns.core.multipleClassSupport(opt.aligment, "dropdown-menu-$1")],
		...
		if (opt.code) {
			...
			if (opt.sample) {
				Object.keys(opt.sample).forEach((sampleKey) => {
					...
					samplecode.push(
						ns.div({
							class: "d-flex justify-content-start py-1 px-3 flex-wrap bg-light",
							attr: {
								"data-bs-toggle": "collapse",
								"aria-expanded": "false",
					...
					samplecode.push(
						ns.div({
							class: "collapse card-body ns-code bg-light pt-0",
							attr: { id: sampleid },
							elems: [
								{
									tag: "code",
							...
							if (data) {
								if (data.length === 1) {
									...
									ns.dlg.box({
										...
										static: false,
										size: "lg",
										elems: ns.div("container p-0", ns.div("d-flex justify-content-center row g-3", prv)),
									});
								}
							}
						})
	...
	ns.input = function (opt) {
		...
		if (opt.before || opt.after || opt.addctl !== null || (opt.type === "number" && opt.numctl === true)) {
			if (opt.flexcontainer) {
				ctl = [ns.div(["d-flex", opt.class], ctl)];
			} else {
				ctl = [
					ns.div(
						[
			...
			if (Array.isArray(opt) || typeof opt === "string") {
				...
				return [
					opt.label
						...
						: null,
					ns.div(["input-group", opt.nowrap ? "flex-nowrap" : null, opt.class], opt.elems),
				].removeEmpty();
			}
		},
		text: function (opt) {
					...
					if (opt.controlstyle === "btn-group") {
						res.push(
							ns.div({
								...
								elems: ns.btngroup(opt.control.map((item) => ns.button(item))),
							})
						);
	...
	ns.listctl = function (opt) {
		...
		if (
			...
			if (opt.title || opt.menu) {
				ctltitle = ns.div("d-flex justify-content-center me-md-2 me-0 mb-2 w-sm-100 w-md-auto", [
					opt.menu
						? opt.option
							? ns.dropdown({
									icon: opt.icon,
						...
						if (opt.size) {
							if (opt.align === "vertical-right") {
								hb = ns.div("row g-0", [
									...
									ns.div([opt.size, "card-footer border-0 p-2"], ns.div(Array.isArray(resHeader) ? resHeader : [resHeader])),
								]);
							} else {
								hb = ns.div("row g-0", [
									ns.div([opt.size, "card-header border-0"], ns.div(Array.isArray(resHeader) ? resHeader : [resHeader])),
									ns.div("col", ns.div("card-body", resBody)),
								]);
							}
						} else {
		...
		container: function (opt) {
			if (Array.isArray(opt)) {
				...
				return {
					...
					class: [
						"navbar",
						opt.expand || opt.expand === "" ? (opt.expand === "" ? "navbar-expand" : `navbar-expand-${opt.expand}`) : null,
	...
	ns.pagination = function (opt) {
		...
		if (opt.total > opt.limit) {
			...
			return ns.div({
				...
				elems: {
					tag: "ul",
					class: ["pagination", opt.weight ? `pagination-${opt.weight}` : null, opt.overflow ? "mx-5" : null].combine(" "),
					elems: li,
				},
			});
		} else {
		...
		container: function (opt) {
			if (Array.isArray(opt)) {
				...
				return ns.tooltip(
					opt.tooltip,
					ns.div({
						class: ["progress", opt.class],
						attr: { id: opt.id },
						style: {
							height: opt.height > 0 ? `${opt.height}px` : null,
						},
						elems: Array.isArray(opt.bar)
...
(function (ns) {
	ns.spinner = function (opt) {
		...
		return {
			...
			attr: { role: "status", id: opt.id, "aria-label": opt.label === "" ? "true" : null },
			style: opt.style,
			elems:
				opt.label === ""
					? null
...
(function (ns) {
	 ...
	 *
	 * ns.div(class,elems)
	 * ns.div(obj:{tag,id,style,attr,class,elems})
	 */
	ns.div = function (...arg) {
		if (arg) {
			...
			// 		tag: arg[0],
			// 		class: arg[1],
			// 		elems: Array.isArray(arg[2]) ? arg[2] : [arg[2]],
			// 	});
			// } else if (arg.length === 2 && (typeof arg[0] === "string" || Array.isArray(arg[0]))) {
			// 	return ns.div({
			// 		tag: "div",
			// 		class: arg[0],
			// 		elems: Array.isArray(arg[1]) ? arg[1] : [arg[1]],
			// 	});
			// } else if (arg.length === 2 && typeof arg[0] === "object") {
			// 	if (!arg[0].elems) {
			// 		arg[0].elems = Array.isArray(arg[1]) ? arg[1] : [arg[1]];
			// 		return ns.div(arg[0]);
			// 	} else {
			// 		console.warn("First arg already have elems");
			// 	}
			// } else if (arg.length === 1 && (typeof arg[0] === "string" || Array.isArray(arg[0]))) {
			// 	ns.div({
			// 		elems: Array.isArray(arg[0]) ? arg[0] : [arg[0]],
			// 	});
			if (arg.length === 2) {
				return ns.div({
					class: arg[0],
	...
	ns.table = function (...arg) {
		if (arg) {
			...
			if (Array.isArray(a)) {
				...
				if (opt.responsive) {
					return ns.div([typeof opt.responsive === "string" ? `tbl-responsive-${opt.responsive}` : "table-responsive"], tbl);
				} else {
					return tbl;
				}
			}
	...
	ns.toast = function (...arg) {
		if (arg && arg.length > 0) {
			if (arg.length === 2 && typeof arg[0] === "string" && typeof arg[1] === "string") {
				...
				var containerquery = [".toast-container", opt.position ? "." + opt.position.split(" ").join(".") : null].combine("");
				...
				if (!container || container.length === 0) {
					ns.build.append(
						...
						ns.div({
							...
							style: { zIndex: "1031" },
							elems: ns.div(["toast-container position-fixed p-3", opt.position], null),
						})
					);
							...
							elems: opt.elems.map(function (i) {
								return {
									...
									elems: {
										tag: "a",
										class: "text-muted",
										attr: { onclick: i.onclick, href: i.href ? i.href : "javascript:void(0)" },
										elems: i.label,
									},
								};
							}),
